...
function Subject(authenticator) {
	...
	/**
	 * @property flag indicating if the current Subject is authenticated or not
	 ...
	 * 
	 * @description Returns <code>true</code> if the <code>Subject</code> has all of the
	 * specified roles, <code>false</code> otherwise.
	 * 
	 * @param {array} roles the application-specific role identifiers to check (usually role ids or role names)
	 * 
 ...
 * exception is thrown; On the contrary authentication informations along with
 * with the submitted principals/credentials are associated with this Subject
 * and the method will return quietly. On successful authentication, the Subject
 * instance is considered authenticated so that the isAuthenticated() method
 * will return true and the getPrincipal() method must return a non-null value
 * and .
 * 
 * @method login
 * 
 * @param token
 *            {UsernamePasswordToken} token the token encapsulating the
 *            subject's principals and credentials to be passed to the
 ...
 * such as <code>Session</code> and authorization data. After this method is
 * called, the Subject is considered 'anonymous' and may continue to be used for
...
/**
 * Returns the principal (primary key, username, ...) that uniquely identify the
 * <code>Subject</code> throughout the entire application, or
 * <code>null</code> if the Subject is not yet known from the application.
 * 
 * @method getPrincipal
...
/**
 * Returns <code>true</code> if the <code>Subject</code> has provided valid
 * credentials, <code>false</code> otherwise.
 * 
 * @method isAuthenticated
 * @return <code>true</code> if the <code>Subject</code> is authenticated,
 *         <code>false</code> otherwise
 * @public
 */
...
/**
 * Returns <code>true</code> if the <code>Subject</code> has the specified
 * role, <code>false</code> otherwise.
 * 
 * @method hasRole
 * @param roleIdentifier
 *            {string} the application-specific role identifier (usually a role
 *            id or role name)
 * @return {boolean} <code>true</code> if the <code>Subject</code> has the
...
/**
 * Checks if the <code>Subject</code> has the specified roles, returning a
 * boolean array indicating which roles are associated
 * 
 * @method hasRoles
 * @param roleIdentifiers
 *            {array} the application-specific role identifiers to check
 *            (usually role ids or role names)
 * @return {array} a boolean array where indices correspond to the index of the
 *         roles in the given identifiers. A <code>true</code> value indicates
 *         the <code>Subject</code> has the role at that index.
 *         <code>false</code> indicates the <code>Subject</code> does not
 *         have the role at that index
 * @public
 */
